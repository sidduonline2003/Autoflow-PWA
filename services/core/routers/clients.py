"""
Clients Router - Client management and portal access
"""
...
async def create_client(client_data: ClientCreationRequest, current_user: dict = Depends(get_current_user)):
    """Create a new client with Firebase Auth account"""
...
async def get_client(client_id: str, current_user: dict = Depends(get_current_user)):
    ...
    return {"id": client_id, **client_data.get("profile", {})}
...
async def update_client(client_id: str, client_data: ClientUpdateRequest, current_user: dict = Depends(get_current_user)):
    ...
    # Update Firebase Auth status if changed
...
async def delete_client(client_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if client_auth_uid:
        auth.update_user(client_auth_uid, disabled=True)
...
async def activate_client(client_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if client_auth_uid:
        auth.update_user(client_auth_uid, disabled=False)
...
async def bulk_update_clients(req: BulkUpdateRequest, current_user: dict = Depends(get_current_user)):
    """Bulk activate or deactivate clients"""
    org_id = current_user.get("orgId")
    if not current_user.get("role") == "admin":
        raise HTTPException(status_code=403, detail="Forbidden")
    
    if req.action not in {"deactivate", "activate"}:
    ...
    batch.commit()
    return {"status": "success", "updatedCount": len(req.clientIds)}
